...
public class WorldGenAetherDungeons extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        {
            for (int l2 = -1; l2 <= 4 + maxHeight; ++l2)
            {
                ...
                for (int i4 = maxHeight; i4 >= -1; --i4)
                {
                    for (int k4 = l1; k4 <= i2; ++k4)
                    {
                        ...
                        else if (worldIn.getBlockState(blockpos1).getBlock().getMaterial().isSolid() && worldIn.getBlockState(blockpos1).getBlock() != Blocks.chest)
                        {
                            ...
                            {
                                worldIn.setBlockState(blockpos1, Blocks.stonebrick.getStateFromMeta(BlockStoneBrick.MOSSY_META), 2);
                            }
                            else
                            {
                                worldIn.setBlockState(blockpos1, Blocks.stonebrick.getDefaultState(), 2);
                            }
                        }
                    }
                }
    ...
    {
        String type = SPAWNERTYPES[p_76543_1_.nextInt(SPAWNERTYPES.length)];
        this.type = type;
        return type;
    }
}
