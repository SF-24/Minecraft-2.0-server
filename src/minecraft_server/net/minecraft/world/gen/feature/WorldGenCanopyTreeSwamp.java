...
public class WorldGenCanopyTreeSwamp extends WorldGenAbstractTree
{
    private static final IBlockState logBlock = Blocks.log2.getDefaultState().withProperty(BlockNewLog.VARIANT, BlockPlanks.EnumType.DARK_OAK);
    private static final IBlockState leaves = Blocks.leaves.getDefaultState().withProperty(BlockNewLeaf.VARIANT, BlockPlanks.EnumType.JUNGLE).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));

    public WorldGenCanopyTreeSwamp(boolean p_i45461_1_)
    {
        super(p_i45461_1_);
    }
            ...
            else
            {
                ...
                for (int j2 = 0; j2 < i; ++j2)
                {
                    ...
                    if (material == Material.air || material == Material.leaves)
                    {
                        ...
                        this.setLog(worldIn, blockpos1.east());
                        this.setLog(worldIn, blockpos1.south());
                        this.setLog(worldIn, blockpos1.east().south());
                    }
                }
                ...
                for (int i3 = -2; i3 <= 0; ++i3)
                {
                    ...
                    {
                        int k4 = -1;
                        this.setLeaves(worldIn, k1 + i3, i2 + k4, l1 + l3);
                        this.setLeaves(worldIn, 1 + k1 - i3, i2 + k4, l1 + l3);
                        this.setLeaves(worldIn, k1 + i3, i2 + k4, 1 + l1 - l3);
                        this.setLeaves(worldIn, 1 + k1 - i3, i2 + k4, 1 + l1 - l3);
                ...
                if (rand.nextBoolean())
                {
                    this.setLeaves(worldIn, k1, i2 + 2, l1);
                    this.setLeaves(worldIn, k1 + 1, i2 + 2, l1);
                    this.setLeaves(worldIn, k1 + 1, i2 + 2, l1 + 1);
                    this.setLeaves(worldIn, k1, i2 + 2, l1 + 1);
                }
                ...
                for (int j3 = -3; j3 <= 4; ++j3)
                {
                    for (int i4 = -3; i4 <= 4; ++i4)
                    {
                        ...
                        {
                            this.setLeaves(worldIn, k1 + j3, i2, l1 + i4);
                        }
                    }
                }
                ...
                for (int k3 = -1; k3 <= 2; ++k3)
                {
                    for (int j4 = -1; j4 <= 2; ++j4)
                    {
                        if ((k3 < 0 || k3 > 1 || j4 < 0 || j4 > 1) && rand.nextInt(3) <= 0)
                        {
                            ...
                            {
                                this.setLog(worldIn, new BlockPos(j + k3, i2 - i5 - 1, l + j4));
                            }
                            ...
                            for (int j5 = -1; j5 <= 1; ++j5)
                            {
                                ...
                                {
                                    this.setLeaves(worldIn, k1 + k3 + j5, i2, l1 + j4 + l2);
                                }
                            }
                            ...
                            for (int k5 = -2; k5 <= 2; ++k5)
                            {
                                for (int l5 = -2; l5 <= 2; ++l5)
                                {
                                    ...
                                    {
                                        this.setLeaves(worldIn, k1 + k3 + k5, i2 - 1, l1 + j4 + l5);
                                    }
                                }
                            }
                        }
    ...
    private void setLog(World p_181639_1_, BlockPos p_181639_2_)
    {
    ...
    private void setLeaves(World worldIn, int p_150526_2_, int p_150526_3_, int p_150526_4_)
    {
